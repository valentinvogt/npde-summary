...
#neq(
  $ frac(partial, partial t) (rho u) - div (kappa (bx) grad u) = f quad upright("in ") tilde(Omega) = Omega times openint(0, T) $,
) <eq:heat-local>
where $u$ is the temperature, $rho$ the heat capacity, $kappa$ the heat
conductivity and $f$ a (time-dependent) heat source/sink. Without the time
derivative, this looks very similar to standard PDEs, for which we know the
transformation into a nice variational problem.

To solve it we still need boundary conditions. Besides the boundary conditions
of the spatial domain --- which are now required for all times --- one also
needs initial conditions over the whole domain at time 0.

#neq(
  $ u (bx , t) & = g (bx , t) quad upright("for ") (bx , t) in partial Omega times openint(0, T)\
...
#lemma(
  number: "9.2.3.8.", "Decay of solutions of parabolic evolutions",
)[
  $colMath("If " f equiv 0, accentcolor)$, the solution $u (t)$ of the heat
  equation @eq:heat-integral-form satisfies
...
Note that this lemma also tells us that if $f$ is time-independent, the solution $u (t)$ converges
exponentially (in time) to the stationary solution (the solution of
@eq:heat-short without the
...
#neq(
  $ bold(M) {frac(dif, dif t) arrow(mu) (t)} + bA arrow(mu) (t) & = arrow(phi) (t)\
  arrow(mu) (0)                                               & = arrow(mu)_0 $,
) <eq:heat-galerkin>
...
$bold(f \() t , bold(u) \) = bA (t) bold(u)$. There is an evolution operator
associated with the ODE, defined as
...
- implicit Euler:$Psi^(t , t + tau) bu = bw \, bw = bu + tau bold(f) \( t + tau , bold(w) \)$
...
#neq(
  $ mat(
    bold(c), bold(frak(A));"", bold(b);delim: #none, augment: #(hline: 1, vline: 1, stroke: 1pt),
...
$ (bold(I)_s times.circle bold(M) + tau bold(frak(A)) times.circle bA) mat(delim: "[", arrow(kappa)_1;dots.v;arrow(kappa)_s) = mat(
  delim: "[", arrow(phi) (t_j + c_1 tau) - bA arrow(mu)^((j));dots.v;arrow(phi) (t_j + c_s tau) - bA arrow(mu)^((j)),
...
To study the stiffness of the method of lines, we first diagonalize it. For
...
so we can study the stability of the general RK-SSM for the scalar case. For $dot(u) = - lambda u$,
with the butcher scheme @eq:butcher we obtain
$Psi_lambda^(t , t + tau) u = S (- lambda tau) u$, with the stability function
$ S (z) = 1 + z bold(b)^top (I - z bold(frak(A)))^(- 1) bold(1) = frac(
  upright("det") (bold(I) - z bold(frak(A)) + z bold(b 1)^top), upright("det") (bold(I) - z bold(frak(A))),
...
#definition(
  number: "9.2.7.46", "L-stability",
)[
  An RK-SSM satisfying the above condition, is called L-stable if its stability
  function satisfies
  $ lr(|S (z)|) < 1 , forall z < 0 upright(" and ") S (- oo) = lim_(z arrow.r - oo) S (z) = 0 $
]

Plugging $- oo$ int $S$ we obtain
$S (- oo) = 1 - bold(b)^top bold(frak(A))^(- 1) bold(1)$, which is equal to zero
if $bold(b)$ is equal to the last row of $bold(frak(A))$.

#theorem(
  number: "9.2.8.5", [Meta-theorem --- Convergence of fully discrete evolution],
)[
  Assume that
...
#neq(
  $ rho (bx) frac(partial^2, partial t^2) u - div (sigma (bx) grad u) = f quad upright("in ") tilde(Omega) $,
) <eq:wave-strong>

Note the similarity to the heat equation @eq:heat-local. Since the wave equation
is a second order ODE $accent(bold(u), dot.double) = bold(f (u))$, two initial
conditions are needed. In addition to the initial conditions @eq:heat-bc-ic, the
initial velocity
...
Furthermore, in the absence of a source term, as in the simple case above, the
solution will stay undamped. This corresponds to
...
#theorem(
  number: "9.3.2.16", "Energy conservation in wave propagation",
)[
  If $u$ solves @eq:wave-variational, then its energy is conserved, in the sense
  that
  $ 1 / 2 m (frac(partial, partial t) u , frac(partial, partial t) u) + 1 / 2 a (u , u) equiv upright("const") $
  where
  $1 / 2 m (frac(partial, partial t) u , frac(partial, partial t) u)$ can be
  understood as the 'kinetic' energy and $1 / 2 a (u , u)$ as the 'potential'
  energy.
]
