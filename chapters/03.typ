...
#theorem(number: "3.1.3.7", "Cea's Lemma")[
...
#mybox("h-Refinement")[
Replace the mesh $cal(M)$ (underlying $V_(0 , h)$) with a finer mesh $cal(M) prime$ (underlying larger discrete trial space $V prime_(0 , N prime)$).
] <concept:refinement>
#v(-1cm)
#mybox("p-Refinement")[
...
#equation(number: "3.2.2.1")[
  #neq($ N = dim S_p^0 (cal(M)) approx p^d h_(cal(M))^(- d) thick arrow.r.double.long h_(cal(M)) / p approx N^(- 1 / d) $) <eq:h-n-relation>
  where $p$ are the dimensions of the local basis functions, e.g., $p = 1$
  for linear basis functions, and $d$ is the dimension of the underlying
  space $Omega$.
]

E.g., in the case where we have $Omega subset bb(R)^2$ and
piecewise linear basis functions, we get $h_(cal(M))^(- 2) approx N$.
...
#theorem(number: "3.3.5.6", "Best approximation error estimates for Lagrangian finite elements")[
  Given a triangular mesh $cal(M)$, if the true solution $u$ is in $H^k (Omega)$, the best approximation error is bounded by
  #neq($ inf_(v_h in cal(S)_p^0 (cal(M))) norm(u - v_h)_(H^1 (Omega)) <= C (h_(cal(M)) / p)^(min { p , k - 1 }) norm(u)_(H^k (Omega)) $)
] <thm:best-approximation-error>

We might not know the constant $C$ and/or $norm(u)_(H^k (Omega))$,
 but we know $p$ and $h_(cal(M))$ as
they are imposed by the choice of function space and mesh. Remember the
concept of refinement: we can adjust these values.
And from Eq. @eq:h-n-relation we know that $h_(cal(M)) \/ p approx N^(- 1 / d)$.
Hence the error displays *algebraic* convergence with rate
$min{ p , k - 1 }\/ d$. What still remains a question is $k$, the
smoothness of the solution $u$.

Another useful result hidden in the lecture notes is
#equation(number: "3.6.3.10", [$L^2$ estimate])[
...
#theorem(number: "3.4.0.2", "Smooth elliptic lifting theorem")[
...
The theorem requires smooth domains, but our meshes will have corners,
so what can be done there? As long as the domain and all cells are
convex, something similar still holds:
#theorem(number: "3.4.0.10", "Elliptic lifting on convex domains")[
...
#tip(
  ...
  [
    ...
    - Sometimes, you may use elliptic regularity results like @thm:elliptic-lifting-convex-domains to find $k=2$, for example.
  ]
)
...
Why is this useful? If $g_F$ can be approximated well in $V_(0 , h)$,
then the output error $lr(|F (u) - F (u_h)|)$ can converge to 0 much
faster than $norm(u - u_h)_a$.
